...
public class CleaningPlanServiceTest {
	...
	public void testGetCleaningPlan() throws Exception {
		...
		when(cleaningPlanRepository.findOne(1L)).thenReturn(expected);
	...
	public void testUpdate() throws Exception {
		...
		CleaningPlanRepository cleaningPlanRepository = mock(CleaningPlanRepository.class);
		when(cleaningPlanRepository.findOne(1L)).thenReturn(saved);
		when(cleaningPlanRepository.save(saved)).thenReturn(expected);
	...
	public void testAddChore() throws Exception {
		...
		CleaningPlanRepository cleaningPlanRepository = mock(CleaningPlanRepository.class);
		when(cleaningPlanRepository.findOne(1L)).thenReturn(cleaningPlan);
		when(cleaningPlanRepository.save(cleaningPlan)).thenReturn(expected);
