...
public class CleaningPlanMapperTest {
	...
	public void testMap() throws Exception {
		...
		ChoreEntity chore1 = spy(new ChoreEntity("chore1", 12345));
		when(chore1.getId()).thenReturn(2L);
		ChoreEntity chore2 = spy(new ChoreEntity("chore2", 12346));
		when(chore2.getId()).thenReturn(3L);
		
		List<ChoreEntity> chores = asList(chore1, chore2);
		CleaningPlanEntity from = spy(new CleaningPlanEntity(chores));
